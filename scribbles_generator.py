...
#     num = 0
#     #pdb.set_trace()
#     for i in sorted(glob.glob("D:/qinchendong/Scribble_supervision/pancreas_tumor_scribble_label/segmentation-0.nii.gz")):
#         print("{} Begin".format(i.split("/")[-1]))
#         itk_data = sitk.ReadImage(i)
#         label = sitk.GetArrayFromImage(itk_data)
#         num_classes = 2  # total segmentation classes
...
#         sitk.WriteImage(itk_scr, i.replace('segmentation-0.nii.gz', 'scirbble.nii.gz'))
...
def process_nii_file(file_path):
    print("{} Begin".format(file_path.split("/")[-1]))
    itk_data = sitk.ReadImage(file_path)
    label = sitk.GetArrayFromImage(itk_data)
    num_classes = 2  # total segmentation classes
    output = generate_scribble(label, tuple([1, num_classes-1]))
    # ignore index for partially cross-entropy loss
    ...
    itk_scr = sitk.GetImageFromArray(output)
    itk_scr.CopyInformation(itk_data)
    #output_file_path = file_path.replace('*.nii.gz', '_scribble.nii.gz')
    sitk.WriteImage(itk_scr, file_path)
    print("{} End".format(file_path.split("/")[-1]))

if __name__ == "__main__":
    folder_path = r"D:\qinchendong\Scribble_supervision\changhai_scribble\mask"
    for file_path in sorted(glob.glob(os.path.join(folder_path, "*.nii.gz"))):
